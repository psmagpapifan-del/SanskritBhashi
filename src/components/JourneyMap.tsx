...
export default function JourneyMap({ currentLang, onSelectChapter, onClose }: JourneyMapProps) {
  ...
  if (!progress || curriculum.length === 0) {
    return <div className="p-8 text-center text-charcoal/50 font-latin">Loading Sanskrit Learning Map...</div>;
  }

  const isChapterUnlocked = (chapterId: number, tier: CurriculumTier): boolean => {
    // Beginner: Chapter 1 is always unlocked. Others require the previous chapter to be completed.
    if (tier === "beginner") {
      if (chapterId === 1) return true;
      return progress.completedChapters.includes(chapterId - 1);
    }
    // Professional: Requires Beginner Gateway Test passed (>= 80%) AND previous chapters
    ...
    // Expert: Requires Professional Gateway Test passed (>= 80%) AND previous chapters
    if (tier === "expert") {
  ...
  const startGatewayTest = (tier: CurriculumTier) => {
    ...
    // Gather composite questions: 1 from every 3 chapters in the tier
    ...
    setTestModal({
      ...
      timeLeft: 120 // 2 minutes timed diagnostic
    });
  };
  ...
  // Check if gateway test is unlockable (all chapters in tier completed)
  ...
  return (
    <div className="bg-cream rounded-3xl p-6 border border-saffron-100 max-h-[85vh] overflow-y-auto select-none">
      {/* Header */}
      ...
      <div className="relative pl-6 border-l-2 border-dashed border-saffron-200 space-y-6 mb-6">
        ...
        {/* Locked Gateway Qualifying Exam Node at the end of the tier */}
        {activeTier !== "expert" && (
        ...
        {testModal && testModal.isOpen && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
            <motion.div
              ...
              className="bg-white border-2 border-saffron-500 rounded-3xl p-6 md:p-8 max-w-lg w-full shadow-2xl relative"
            >
              {/* Header */}
